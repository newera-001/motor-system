...
class Minitaur(object):
    ...
    def __init__(self,
        ...
        self._foot_link_ids = []
        self._environment_parameters_value_dict = {}
    ...
    def GetMotorAngles(self):
        """Gets the eight motor angles. （laikago是12个）
    ...
    def ApplyAction(self, motor_commands, motor_control_mode):
        ...
        self.last_action_time = self._state_action_counter * self.time_step           # 当前的累计时间
        control_mode = motor_control_mode

        if control_mode is None:
            control_mode = self._motor_control_mode

        motor_commands = np.asarray(motor_commands)                                   # 电机的输入值
    ...
    def GetBaseMasses(self):
        self.chassis_masses = []

        for chassis_id in self._chassis_link_ids:
            mass = self._pybullet_client.getDynamicsInfo(self.quadruped,
                                                         chassis_id)
            self.chassis_masses.append(mass[0])
    ...
    def GetLegMasses(self):
        ...
        for leg_id in self._leg_link_ids:
            mass = self._pybullet_client.getDynamicsInfo(self.quadruped,
                                                         leg_id)
            self.leg_masses.append(mass[0])

        for link_id in self._motor_link_ids:
            mass = self._pybullet_client.getDynamicsInfo(self.quadruped,
                                                         link_id)
            self.motor_masses.append(mass[0])
    ...
    def GetBaseInertias(self):
        self.chassis_inertias = []

        for chassis_id in self._chassis_link_ids:
            self.chassis_inertias.extend(list(self._pybullet_client.getDynamicsInfo(self.quadruped,
                                                                               chassis_id)[2]))
    ...
    def GetLegtInertias(self):
        ...
        for leg_id in self._leg_link_ids:
            mass = self._pybullet_client.getDynamicsInfo(self.quadruped,
                                                         leg_id)
            self.leg_inertias.extend(list(mass[2]))

        for link_id in self._motor_link_ids:
            mass = self._pybullet_client.getDynamicsInfo(self.quadruped,
                                                         link_id)
            self.motor_inertias.extend(list(mass[2]))
    ...
    def GetFootFriction(self):
        ...
        for link_id in self._foot_link_ids:
            self.foot_friction.append(self._pybullet_client.getDynamicsInfo(self.quadruped,link_id)[1])
    ...
    def GetFootRestitution(self):
        self.foot_retitution = []

        for link_id in self._foot_link_ids:
            self.foot_retitution.append(self._pybullet_client.getDynamicsInfo(self.quadruped,link_id)[5])
    ...
    def GetJointFriction(self):
        ...
        for knee_joint_id in self._foot_link_ids:
            self.joint_friction.append(self._pybullet_client.getJointInfo(self.quadruped,knee_joint_id)[7])
    ...
    def GetBatteryVoltage(self):
        return self._motor_model.get_voltage()

    def SetMotorViscousDamping(self, viscous_damping):
        self._motor_model.set_viscous_damping(viscous_damping)

    # motor_friction
    def GetMotorViscousDamping(self):
        return self._motor_model.get_viscous_dampling()
    ...
    def GetMotorStrengthRatios(self):
        return self._motor_model._strength_ratios
    ...
    def GetEnvironmentParameters(self):
        self.GetBaseMasses()
        self.GetLegMasses()
        self.GetBaseInertias()
        self.GetLegtInertias()
        self.GetFootFriction()
        control_latency = self.GetControlLatency()
        self.GetJointFriction()
        motor_friction = self.GetMotorViscousDamping()
        self.GetFootRestitution()
        voltage = self.GetBatteryVoltage()
        motor_strength = self.GetMotorStrengthRatios()

        self._environment_parameters_value_dict['base_masses'] = self.chassis_masses
        self._environment_parameters_value_dict['leg_masses'] = self.leg_masses
        self._environment_parameters_value_dict['motor_masses'] = self.motor_masses
        self._environment_parameters_value_dict['chassis_inertias'] = self.chassis_inertias
        self._environment_parameters_value_dict['leg_inertias'] = self.leg_inertias
        self._environment_parameters_value_dict['motor_inertias'] = self.motor_inertias
        self._environment_parameters_value_dict['foot_friction'] = self.foot_friction
        self._environment_parameters_value_dict['latency'] = control_latency
        self._environment_parameters_value_dict['joint_friction'] = self.joint_friction
        self._environment_parameters_value_dict['motor_friction'] = motor_friction
        self._environment_parameters_value_dict['foot_retitution'] = self.foot_retitution
        self._environment_parameters_value_dict['voltage'] = voltage
    ...
    def ProcessAction(self, action, substep_count):
        ...
        Returns:
          If interpolation(插值) is enabled, returns interpolated action depending on
