...
QUATERNION_IDENTITY = np.array([0., 0., 0., 1.], dtype=np.float64)
...
def Vector3RandomUniform(low=VECTOR3_0, high=VECTOR3_1):
    ...
    Returns:
      A 3D vector in a numpy array.
    """
    random_x = np.random.uniform(low[0], high[0])
    random_y = np.random.uniform(low[1], high[1])
    random_z = np.random.uniform(low[2], high[2])
    random_v3 = np.array([random_x, random_y, random_z])
    return random_v3
...
def Vector3RandomUnit():
    ...
    sin_latitude = np.random.uniform(low=-1., high=1.0)
...
def QuaternionNormalize(q):
    ...
    if np.isclose(q_norm, 0.):
        raise ValueError('Quaternion may not be zero in QuaternionNormalize: |q| = %f, q = %s' %
                         (q_norm, q))
    return q / q_norm
