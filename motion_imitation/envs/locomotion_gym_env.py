...
class LocomotionGymEnv(gym.Env):
    ...
    def __init__(self,
        ...
        if self._is_render:
            ...
            # 这和渲染是是否现实参考的模型的机器人有关（有一个参考机器人的影子 在训练时）
            ...
            # 这个方法能让用户自己定义调节参数 这个delay对应渲染窗口的delay：xxxx  作用应该是让参考机器人的动作相应的延迟执行吧
    ...
    def _build_action_space(self):
        ...
        if motor_mode == robot_config.MotorControlMode.HYBRID:
            ...
            # Position mode, 本次使用的就是这个模式
            action_upper_bound = []
            action_lower_bound = []

            # _robot_class.ACTION_CONFIG 返回的是一个list 值的类型是：
            # ScalarField 包含三个属性 name、lower_bound、upper_bound
            action_config = self._robot_class.ACTION_CONFIG
