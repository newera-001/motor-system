...
from motion_imitation.envs.utilities import laikago_env_randomizer_config
...
class ControllableEnvRandomizerFromConfig(
    ...
    """A randomizer that change the laikago_gym_env during every reset."""
    ...
    def __init__(self,
        ...
        if config is None:
            config = "all_params"
        try:
            config = getattr(laikago_env_randomizer_config, config)
        ...
        self._environment_param_value_dict = {}                             # 这个才是用来做adaptation的
    ...
    def randomize_env(self, env):
        ...
        Args:
          env: A minitaur gym environment.(这里传进来的是 locomotion_gym_env)
        """
        ...
        self._environment_param_value_dict = env.get_env_parameters()
    ...
    def get_environment_parameters(self):
        return copy.deepcopy(self._environment_param_value_dict)

    def set_env_from_randomization_parameters(self, env,
                                              randomization_parameters):
        # because hard_reset might change previously
    ...
    def _randomize_control_step(self,
        ...
        if parameters is None:
            ...
            sample = parameters
        self._randomization_param_value_dict["control step"] = sample               # 存到随机参数字典里 需要时掉出来用
        ...
        env.set_time_step(randomized_control_step)                                  # 将标准化好的参数赋值到仿真环境里
        if self._verbose:
    ...
    def _randomize_masses(self,
        ...
        self._randomization_param_value_dict["mass"] = sample                       # 存到随机参数字典里 需要时掉出来用
        randomized_mass_ratios = (sample - self._param_bounds[0]) / (
                self._param_bounds[1] -
                self._param_bounds[0]) * (upper_bound - lower_bound) + lower_bound

        base_mass = minitaur.GetBaseMassesFromURDF()                                # 这是body的
        random_base_ratio = randomized_mass_ratios[0]
        randomized_base_mass = random_base_ratio * np.array(base_mass)
        minitaur.SetBaseMasses(randomized_base_mass)                                # 将标准化好的参数赋值到仿真环境里
        if self._verbose:
            tf.logging.info("base mass is: {}".format(randomized_base_mass))

        leg_masses = minitaur.GetLegMassesFromURDF()                                # 这是link的
